...
public class QCStore {
	...
	// Brahmith Transactions
        @Test(priority = 13, groups = "NextPayDate")
	  public static void Payfrequency_Biweekly() throws Exception {
			...
			for (int row = 2; row <= lastrow+1; row++) {
				...
				if (RunFlag.equals("Y")) {
					...
					BAdminStartDate.toStartdate(SSN, AppURL);
	...
	@Test(priority = 14, groups = "NextPayDate1")
	public static void Payfrequency_Weekly() throws Exception {
        ...
        @Test(priority = 14, groups = "NextPayDate")
       public static void Payfrequency_Monthly() throws Exception {
		

			test = reports.startTest("Monthly_frequency","Frequency verification");
