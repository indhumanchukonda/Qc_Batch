...
public class RQCCSRSignatureLoan extends QCStore{
	...
	public static void signatureloan(String SSN,String AppURL) throws Exception
	{
		...
		{	
			...
			if (SSN.equals(RegSSN)) {
				...
				stateProductType=TestData.getCellData(sheetName,"ProductTypee",row);
	...
	{							
			...
			for(int i=2;i<=n;i++){
				...
				if(Pname.equalsIgnoreCase(ProductType))
				{
						...
						test.log(LogStatus.PASS, "Selected check box as "+stateProductType);
